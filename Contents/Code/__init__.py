...
def MainMenu():
    ...
    oc.add(
        DirectoryObject(
            key=Callback(
                Soaps, title2=u'Все сериалы', filter='all'
            ),
            title=u'Все сериалы'
        )
    )
    oc.add(
        DirectoryObject(
            key=Callback(
                Soaps, title2=u'Я смотрю', filter='watching'
            ),
            title=u'Я смотрю'
        )
    )
    oc.add(
        DirectoryObject(
            key=Callback(
                Soaps, title2=u'Новые эпизоды', filter='unwatched'
            ),
            ...
            dir.add(
                TVShowObject(
                    key=Callback(
                        ...
                        filter=filter, unwatched=(filter == 'unwatched')
                    ),
                    rating_key=str(id),
                    title=title,
                    summary=summary,
        ...
        dir.add(
            SeasonObject(
                key=Callback(
                    ...
                    filter=filter, soap_title=soap_title,
                    unwatched=unwatched
                ),
                episode_count=len(
